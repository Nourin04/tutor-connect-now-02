...
function TeacherOnboarding() {
  ...
  async function saveSubjects() {
    ...
    const { error } = await supabase
      .from("teacher_subjects")
      .insert(cleaned.map((s) => ({ ...s, teacher_id: userId })));
    if (error) return toast.error(error.message);
            ...
            <Section title="Personal info">
              <Grid>
                ...
                <Field label="Area / locality">
                  <Input value={s1.area} onChange={(e) => setS1({ ...s1, area: e.target.value })} />
                </Field>
                <Field label="Profile photo URL (optional)">
                  <Input
                    value={s1.avatar_url}
                    onChange={(e) => setS1({ ...s1, avatar_url: e.target.value })}
                    placeholder="https://…"
                  />
                </Field>
              </Grid>
            ...
            <Section title="Qualifications">
              <Grid>
                <Field label="Highest degree">
                  <Input
                    value={s2.highest_degree}
                    onChange={(e) => setS2({ ...s2, highest_degree: e.target.value })}
                ...
                <Field label="University / Institution">
                  <Input
                    value={s2.university}
                    onChange={(e) => setS2({ ...s2, university: e.target.value })}
                  />
                </Field>
                <Field label="Years of teaching experience">
                  <Input
                    type="number"
                    min={0}
                    value={s2.years_experience}
                    onChange={(e) => setS2({ ...s2, years_experience: Number(e.target.value) })}
                  />
                </Field>
              </Grid>
              <TagInput
            ...
            <Section title="Subjects you teach">
              <div className="space-y-3">
                {subjects.map((s, i) => (
                  <div
                    key={i}
                    className="grid items-end gap-3 rounded-xl border border-border bg-background p-3 sm:grid-cols-[1fr_1fr_1fr_auto]"
                  >
                    <SelectRow
                      label="Subject"
                      value={s.subject}
                    ...
                    <SelectRow
                      label="Board"
                      value={s.board}
                      onChange={(v) => updateSubject(i, "board", v)}
                      options={BOARDS}
                    />
                    <Button
                      variant="ghost"
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setSubjects([...subjects, { subject: "", level: "", board: "" }])}
                >
          ...
          {step === 3 && (
            <Section title="Availability & fees">
              <Field label="About you (short bio)">
                <Textarea
                  value={s4.bio}
                  onChange={(e) => setS4({ ...s4, bio: e.target.value })}
                  maxLength={1000}
                  rows={4}
                  placeholder="Briefly describe your teaching style…"
                />
              </Field>
              <ChipPicker
                label="Available days"
                options={DAYS}
                values={s4.available_days}
                onChange={(v) => setS4({ ...s4, available_days: v })}
              />
              <ChipPicker
                label="Time slots"
                options={SLOTS}
                values={s4.time_slots}
                onChange={(v) => setS4({ ...s4, time_slots: v })}
              />
              <Grid>
                ...
                <Field label="Fee min (₹/hr)">
                  <Input
                    type="number"
                    min={0}
                    value={s4.fee_min}
                    onChange={(e) => setS4({ ...s4, fee_min: Number(e.target.value) })}
                  />
                </Field>
                <Field label="Fee max (₹/hr)">
                  <Input
                    type="number"
                    min={0}
                    value={s4.fee_max}
                    onChange={(e) => setS4({ ...s4, fee_max: Number(e.target.value) })}
                  />
                </Field>
              </Grid>
              <TagInput
                ...
                values={s4.languages}
                onChange={(v) => setS4({ ...s4, languages: v })}
                placeholder="e.g. English, Hindi, Tamil"
              />
              <NextRow
                onBack={() => setStep(2)}
                onNext={saveAvailability}
